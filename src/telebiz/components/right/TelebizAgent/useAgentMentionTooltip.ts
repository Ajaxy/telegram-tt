...
const RE_MENTION_SEARCH = /(^|\s)@([-_\w]*)$/i;
...
export default function useAgentMentionTooltip(
  ...
  useEffect(() => {
    ...
    // Get user IDs from contacts and recent chats
    const contactIds = global.contactList?.userIds || [];
    const allUserIds = Object.keys(usersById);
    const userIds = contactIds.length > 0 ? contactIds : allUserIds.slice(0, 100);
  ...
  const insertMention = useLastCallback((user: ApiUser) => {
    ...
    onInsertMention(beforeMention, username, afterMention);
    setFilteredUsers(undefined);
  });
